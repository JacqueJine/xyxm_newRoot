...
import { getSchoolList, updateSchool } from "@/api/school";
...
export default {
  ...
  methods: {
    ...
    getData(id) {
      getSchoolList(
        this.$qs.stringify({ where: { id: id }, page: 1, limit: 10 })
      ).then((res) => {
        this.form.school_name = res.result.data[0].school_name;
        this.form.school_address = res.result.data[0].school_address;
        this.loading = false;
      });
    },
